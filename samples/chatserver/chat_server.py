...
def login_handler(client, request):
    ...
    print "user %s login to server %s bind session %s" % (user_id, server_id,
                                                          session_id)
    ...
    resp = "You have login to Server(%s), bind a Session(%s)" % (server_id,
                                                                 session_id)
...
class ChatServer(Server):
    def __init__(self, protocol, client_cls):
        super(ChatServer, self).__init__(protocol=protocol,
